...
In order to test the Spring Boot application locally, before packing it into a
rock, fetch the ``spring-boot`` application from the Spring Initializr.
We'll also install the Java Development Kit (JDK) to run and build the
application.
...
``{"timestamp":"2025-02-25T08:39:37.197+00:00","status":404,"error":"Not Found","path":"/"}``.

The Spring Boot application looks spring-bootod, so let's stop it for now
with :kbd:`Ctrl` + :kbd:`C`.
...
..  code-block:: text
    ...
    2025-02-25T08:39:21.781Z  INFO 4048 --- [spring-boot] [           main] com.example.spring_boot.Application      : Starting Application using Java 21.0.6 with PID 4048 (/home/ubuntu/target/classes started by ubuntu in /home/ubuntu)
    2025-02-25T08:39:21.782Z  INFO 4048 --- [spring-boot] [           main] com.example.spring_boot.Application      : No active profile set, falling back to 1 default profile: "default"
    2025-02-25T08:39:22.156Z  INFO 4048 --- [spring-boot] [           main] o.s.b.w.embedded.tomcat.TomcatWebServer  : Tomcat initialized with port 8080 (http)
...
Now we have a fully functional rock for a Spring Boot application! This
concludes the first part of this tutorial, so we'll stop the container and
remove the respective image for now:
...
Start by opening the ``src/main/java/com/example/spring_boot/Application.java``
file in a text editor and update the code to look like the following:

.. literalinclude:: code/spring-boot/Application.java
    :language: java
